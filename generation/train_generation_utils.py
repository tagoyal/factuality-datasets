...
from transformers.modeling_bart import PretrainedBartModel, BartForConditionalGeneration, BartModel, BartConfig
from transformers.modeling_gpt2 import GPT2LMHeadModel, GPT2PreTrainedModel
import torch.nn.functional as F
from torch import nn
from loss_dropper import LossDropper
from transformers.modeling_utils import top_k_top_p_filtering, calc_banned_bad_words_ids, BeamHypotheses, calc_banned_ngram_tokens
...
class InputFeatures(object):
    def __init__(self, input_ids, decoder_ids, attention, decoder_attention, labels, input_prompt_len):
...
def convert_examples_to_features_bart_p(examples, tokenizer, max_length=128):
    features = []

    max_length_decoder = 512
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        index_now = 1
        for (word_index, word) in enumerate(output.split(' ')):
            if word == '':
                continue
            word_tokens = tokenizer.tokenize(word, add_prefix_space=True)
            if len(word_tokens) > 0:
                decoder_tokens.extend(word_tokens)
                index_map[word_index] = [index_now + x for x in range(len(word_tokens))]
                index_now += len(word_tokens)

        decoder_tokens.extend(tokenizer.tokenize(eos_token, add_prefix_space=True))
        decoder_ids = tokenizer.convert_tokens_to_ids(decoder_tokens)

        assert len(decoder_tokens) == len(decoder_ids), 'why wont you work'
        ...
        if len(output_ids) > 0:
            for id in output_ids:
                if id == '':
                    continue
                if int(id) in index_map.keys():
                    output_ids_new.extend(index_map[int(id)])

        if len(decoder_ids) > max_length_decoder:
            decoder_ids = decoder_ids[:max_length_decoder]

        padding_length_b = max_length_decoder - len(decoder_ids)
        decoder_attention_mask = [1] * len(decoder_ids) + ([0] * padding_length_b)
        decoder_ids = decoder_ids + ([pad_id] * padding_length_b)

        decoder_output_ids = copy.deepcopy(decoder_ids)
        for id in output_ids_new:
            if id >= len(decoder_ids):
                continue
            decoder_output_ids[id] = pad_id
        ...
        features.append(InputFeatures(input_ids=input_ids,
                                      ...
                                      input_prompt_len=None))
    print(len(features))
    return features


def load_and_cache_examples_bart_partial(args, tokenizer, evaluate):
    if evaluate:
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        features = convert_examples_to_features_bart_p(
            examples,
            tokenizer,
    ...
    decoder_labels = torch.tensor([f.labels for f in features], dtype=torch.long)

    dataset = TensorDataset(input_ids, input_attention_mask, decoder_ids, decoder_attention_mask, decoder_labels)
...
class BartForConditionalGenerationCustom(PretrainedBartModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
        base_model = BartModel(config)
        self.model = base_model
        self.dropper = LossDropper(dropc=0.7)
    ...
    def forward(
        ...
        if not generate:
            if partial_loss:
                lm_labels = shift_tokens_left(lm_labels, 1)  # hardcoded, should fix
            else:
